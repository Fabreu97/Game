#ifndef INCLUDED_LIST_TEMPLATE
#define INCLUDED_LIST_TEMPLATE
...
public:
    ...
    const bool pushFront (TL* d);
    TL* front ();
    TL* back ();
    void deleteData(const unsigned long int indice); //  REVIEW
    void deleteData(TL* type);
    void pop();                       //  REVIEW
    const bool empty () const { return ( this->getSize() ? true : false ); }
...
template<class TL>
template<class TE>
List<TL>::Element<TE>::Element(TE* type) {
    data = type;
    prev = NULL;
    next = NULL;
    data = NULL;
...
template<class TL> const bool List<TL>::pushFront(TL* d) {
    if (d != NULL) {
        Element<TL>* e = new Element<TL> (d);
        if (first != NULL) {
            first->setPrev(e);
            e->setPrev(first);
            first = e;
...
template <class TL> TL* List<TL>::front () {
    if(first != NULL) {
...
template <class TL> TL* List<TL>::back () {
    return ( this->last != NULL ? last->getData() : NULL );
}
